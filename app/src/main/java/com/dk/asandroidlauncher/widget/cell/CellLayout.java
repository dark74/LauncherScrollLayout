package com.dk.asandroidlauncher.widget.cell;
...
public class CellLayout extends ViewGroup implements View.OnDragListener {
    ...
    //判断是否可以放置cell
    private boolean checkIsEnough(boolean[][] myCellHolds, int startColumnIndex, int startRowIndex, int widthCellNums, int heightCellNums) {
        if (startColumnIndex + widthCellNums > columns || startRowIndex + heightCellNums > rows) {
            return false;
        }
        for (int i = startColumnIndex; i < startColumnIndex + widthCellNums; i++) {
            for (int j = startRowIndex; j < startRowIndex + heightCellNums; j++) {
                if (myCellHolds[j][i]) {
                    return false;
                }
            }
    ...
    public static class Cell {
        ...
        private int expectColumnIndex = -1;//计算出的可摆放x索引的位置
        private int expectRowIndex = -1;//计算出的可摆放y索引的位置
